...
/**
 * Single catalog of every email the platform can send for an organization.
 *
 * There is exactly ONE HTML template (`renderEmailTemplate` in
 * settings/email-template.ts). Each entry below only provides the *content*
 * (subject, title, body, CTA, footer) that is dropped into that template, so
 * every email automatically picks up the organization's logo, name, colors and
 * footer — no per-email templates.
 */

export type EmailType = 'invitation' | 'passwordReset' | 'otp' | 'notification' | 'invoice' | 'report';
