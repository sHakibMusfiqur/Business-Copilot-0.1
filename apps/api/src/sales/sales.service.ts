...
export class SalesService {
  ...
  async create(orgId: string, userId: string, dto: CreateSaleDto) {
    ...
    const itemsData = this.buildSaleItems(dto.items, products);
    ...
    if (total < 0) {
      throw new BadRequestException('Sales order total cannot be negative');
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        await this.auditService.record({
          ...
          entityId: sale.id,
          status: 'SUCCESS',
          metadata: { orderNumber: sale.orderNumber, total: sale.total },
        });
  ...
  async softDelete(orgId: string, userId: string, saleId: string) {
    ...
    await this.auditService.record({
      ...
      entityId: saleId,
      status: 'SUCCESS',
      metadata: { orderNumber: sale.orderNumber },
    });
