...
function safeColor(value: string | undefined): string {
  return value && HEX_RE.test(value) ? value : FALLBACK_PRIMARY;
}
...
function buildLogo(brand: BrandEmailContext): string {
  ...
  const initials = (brand.companyName || 'Business Copilot')
    ...
    .join('')
    .toUpperCase() || 'BC';
  return `<span style="display:inline-block; height:40px; line-height:40px; padding:0 16px; border-radius:10px; background:${safeColor(brand.primaryColor)}; color:#ffffff; font-size:16px; font-weight:700; letter-spacing:0.5px;">${escapeHtml(initials)}</span>`;
}
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 8px 24px rgba(15, 23, 42, 0.08);">
          ...
          <tr>
            <td style="padding:16px 32px 24px; border-top:1px solid #e2e8f0;">
              <p style="margin:0; font-size:12px; color:#94a3b8; text-align:center;">${companyName}${content.footer ? ` &middot; ${escapeHtml(content.footer)}` : ''}</p>
            </td>
          </tr>
        </table>
      </td>
