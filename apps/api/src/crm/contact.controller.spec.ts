...
function buildController(getter: (orgId: string, userId: string) => Promise<string[]>) {
  ...
  const controller = new ContactController(contact, workspaceAccess, enforcer);
...
type EndpointSpec = {
  name: string;
  method: keyof ContactController;
  permissions: string[];
  serviceCall: (ctx: { controller: ContactController; contact: ContactService }) => Promise<unknown>;
  serviceMock: (ctx: { controller: ContactController; contact: ContactService }) => jest.Mock;
};

const ENDPOINTS: EndpointSpec[] = [
  {
    name: 'contacts findAll',
    method: 'findAll',
    permissions: ['crm.read'],
    serviceCall: ({ controller }) => controller.findAll(makeUser(), { page: 1 } as never),
    serviceMock: ({ contact }) => contact.findAll as jest.Mock,
  },
  {
    name: 'contacts findById',
    method: 'findById',
    permissions: ['crm.read'],
    serviceCall: ({ controller }) => controller.findById(makeUser(), 'contact-1'),
    serviceMock: ({ contact }) => contact.findById as jest.Mock,
  },
  {
    ...
    serviceMock: ({ contact }) => contact.create as jest.Mock,
  },
  {
    name: 'contacts update',
    method: 'update',
    permissions: ['crm.update'],
    serviceCall: ({ controller }) => controller.update(makeUser(), 'contact-1', {} as never),
    serviceMock: ({ contact }) => contact.update as jest.Mock,
  },
  {
    name: 'contacts delete',
    method: 'remove',
    permissions: ['crm.delete'],
    serviceCall: ({ controller }) => controller.remove(makeUser(), 'contact-1'),
    serviceMock: ({ contact }) => contact.softDelete as jest.Mock,
  },
];

describe('ContactController (additive Business OS enforcement across CRM)', () => {
  it('retains existing @Permissions metadata on every endpoint', () => {
    for (const endpoint of ENDPOINTS) {
      const descriptor = Object.getOwnPropertyDescriptor(ContactController.prototype, endpoint.method);
  ...
  it('ADMIN and USER role names never grant CRM access by themselves', async () => {
    ...
    expect(ctx.contact.findAll).not.toHaveBeenCalled();
  });
  ...
  it('organizationId always comes from the authenticated user.organizationId', async () => {
    ...
    expect(ctx.contact.findAll).toHaveBeenCalledWith('org-42', expect.anything());
  });
  ...
  it('ADMIN and USER role mappings continue to work for authorized users', async () => {
    ...
    expect(ctx.contact.findAll).toHaveBeenCalledTimes(2);
  });
...
describe('ContactController (Nest DI wiring)', () => {
  it('resolves the controller through DI with no manual instantiation of Business OS services', async () => {
    ...
    try {
      const controller = moduleRef.get(ContactController);
