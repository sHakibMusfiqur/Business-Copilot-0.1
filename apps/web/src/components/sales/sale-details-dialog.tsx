...
import type { Sale, SalesStatus } from './sales-types';
      ...
      <div className="relative z-50 w-full max-w-3xl rounded-xl border bg-card p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${statusStyle[sale.status]}`}>
          ...
          <div>
            ...
            <p className="text-sm">{formatDate(sale.orderDate)}</p>
          </div>
          <div>
          ...
          <table className="w-full">
            ...
            <tfoot>
              <tr className="border-t bg-muted/30">
                ...
                <td className="p-3 text-sm text-right">{formatCurrency(Number(sale.subtotal))}</td>
              </tr>
              {Number(sale.discount) > 0 && (
                <tr>
                  <td colSpan={5} className="p-3 text-sm text-right text-muted-foreground">Discount</td>
                  <td className="p-3 text-sm text-right text-muted-foreground">-{formatCurrency(Number(sale.discount))}</td>
                </tr>
              )}
              {Number(sale.tax) > 0 && (
                <tr>
                  <td colSpan={5} className="p-3 text-sm text-right text-muted-foreground">Tax</td>
                  <td className="p-3 text-sm text-right text-muted-foreground">{formatCurrency(Number(sale.tax))}</td>
                </tr>
              )}
              {Number(sale.shippingCost) > 0 && (
                <tr>
                  <td colSpan={5} className="p-3 text-sm text-right text-muted-foreground">Shipping</td>
                  <td className="p-3 text-sm text-right text-muted-foreground">{formatCurrency(Number(sale.shippingCost))}</td>
                </tr>
              )}
              <tr className="border-t">
                <td colSpan={5} className="p-3 text-sm text-right font-semibold">Total</td>
                <td className="p-3 text-sm text-right font-semibold">{formatCurrency(Number(sale.total))}</td>
              </tr>
            </tfoot>
          </table>
        </div>
