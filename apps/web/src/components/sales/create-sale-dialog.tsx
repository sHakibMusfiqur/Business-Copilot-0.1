...
import { createSale, getCustomers, getProducts } from '@/lib/api';
...
export function CreateSaleDialog({ open, onClose, onCreated }: CreateSaleDialogProps) {
  ...
  function handleSubmit() {
    ...
    const result = createSchema.safeParse({ customerId, items: parsedItems });
    if (!result.success) {
              ...
              <div key={item.id} className="flex items-end gap-3 mb-3 p-3 rounded-lg border bg-muted/30">
                ...
                <div className="w-20">
                  ...
                  <Input
                    ...
                    step="0.01"
                    value={item.discount}
                    onChange={(e) => updateRow(item.id, 'discount', e.target.value)}
                  />
                </div>
                <div className="w-20">
                  ...
                  <Input
                    ...
                    step="0.01"
                    value={item.tax}
                    onChange={(e) => updateRow(item.id, 'tax', e.target.value)}
                  />
                </div>
                <Button
                  variant="ghost"
