...
import { updateLead, getOrganizationUsers } from '@/lib/api';
import type { Lead, LeadListItem, LeadStatus } from './crm-types';
...
export function EditLeadDialog({ lead, open, onClose, onUpdated }: EditLeadDialogProps) {
  ...
  useEffect(() => {
    if (leadData) {
      ...
      setEstimatedValue(leadData.estimatedValue ? String(leadData.estimatedValue) : '');
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      if (!leadData) throw new Error('No lead selected');
      return updateLead(leadData.id, {
        name,
      ...
      <div className="relative z-50 w-full max-w-2xl rounded-xl border bg-card p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <Input id="edit-company" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Acme Corp" />
          </div>
          <div className="col-span-2 sm:col-span-1">
            <Label htmlFor="edit-email">Email</Label>
            <Input id="edit-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@acme.com" />
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <Input id="edit-source" value={source} onChange={(e) => setSource(e.target.value)} placeholder="Website, Referral, ..." />
          </div>
          <div className="col-span-2 sm:col-span-1">
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <Input id="edit-estimatedValue" type="number" min="0" step="0.01" value={estimatedValue} onChange={(e) => setEstimatedValue(e.target.value)} />
          </div>
          <div className="col-span-2 sm:col-span-1">
